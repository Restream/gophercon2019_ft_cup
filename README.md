...
# RTK/Gophercon-2019 Fulltext search cup

## Конкурсная задача

Вне зависимости от размера и количества контента любой сервис показывающий фильмы, сериалы, TV трансляции сталкивается с необходимостью - дать пользователям полнотекстовый поиск по контенту. Чем качественее и быстрее
полнотекстовый поиск отвечает на запросы - тем чаще и больше пользователи будут пользоваться сервисом.

Участникам конкурса предлагается задача - реализовать приложение поиска контента. Приложение должно как можно быстрее выдавать результаты поиска по базе с контентом (фидьмы/программа телепередач) на запросы от фронтенда - по сути предоставлять для фронтенда функциональное API
...
1. Подготовь Docker образ с решением конкурсной задачи. В нем должен быть запущен сервер приложения, принимающий запросы на порту 8080. Пример Dockerfile для сборки контнейнера можно посмотреть [тут](Dockerfile)
...
После создания MR будет автоматически запущен тестый пайплайн обстрела в сервисе travis-ci, и через несколько минут будет получен результат тестирования.
...
- файл `epg.json` - программа телепереда
- файл `media_items.json` - список медиаконтента

Объем и содержание тестовых данных может меняться в процессе провеления конкурса, но они будут одинаковыми для всех участников.
...
При выборе победителей кроме техничесих харрактеристик будут оцениваться исходные тексты решения - мы предполагаем, что решениие будет "честно" реализовывть требуемый функционал, не пытаясь подстроиться под конкретный датасет, подгоняя ответы под ожидания системы проверки :)
...
| `packages`                                   | []int64  | идентификторы пакетов, в которые входит контент                                         |
...
Полнотекстовый поиск должен находить нечеткие совпдаения поискового запроса, с контентом в базе. Ожидается поддержка функциональности:
