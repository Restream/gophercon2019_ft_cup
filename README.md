...
Вне зависимости от размера и количества контента любой сервис показывающий фильмы, сериалы, TV трансляции сталкивается с необходимостью - дать пользователям полнотекстовый поиск по контенту. Чем качественнее и быстрее
полнотекстовый поиск отвечает на запросы - тем чаще и больше пользователи будут пользоваться сервисом.

Участникам конкурса предлагается задача - реализовать приложение поиска контента. Приложение должно как можно быстрее выдавать результаты поиска по базе с контентом (фильмы/программа телепередач) на запросы от фронтенда - по сути предоставлять для фронтенда функциональное API
